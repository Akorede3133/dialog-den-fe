...
const VideoCall = () => {
  ...
  return (
    <div className="h-screen bg-white dark:bg-bg-dark lg:h-[25rem] flex flex-col justify-between lg:justify-star items-center gap-4 w-full lg:w-[30rem] lg:right-[10px] lg:top-[10px] lg:rounded-[1rem] overflow-hidden  py-5 absolute z-30 shadow-[0_0_10px_rgba(0,0,0,0.2)]">
      ...
      <div className=' text-center absolute'>
        <p className={`text-2xl ${callerBigDisplay ? 'text-text-primary-dark' : ' text-white'}`}>{outGoingVideoCall?.username}</p>
        <span className={`text-sm ${callerBigDisplay ? 'text-text-primary dark:text-text-primary-dark' : ' text-white'}`}>{onGoingCall ? formatDuration(callDuration) : 'calling...'}</span>
      </div>
      {
